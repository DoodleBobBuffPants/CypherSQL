...
public class MainTest {
    ...
    public void invalidTranslatorThrowsDuringDump() {
        NoTranslatorFoundException exception = assertThrows(NoTranslatorFoundException.class, () -> Main.main(new String[] { "-dump", "-source", "source-file", "-target", "target-file", "-database", "invalid" }));
    ...
    public void invalidTranslatorThrowsDuringExecute() {
        NoTranslatorFoundException exception = assertThrows(NoTranslatorFoundException.class, () -> Main.main(new String[] { "-execute", "-query", "SELECT 1", "-database", "invalid" }));
        assertEquals("No translator found to handle database invalid", exception.getMessage());
    }

    @Test
    public void invalidTranslatorThrowsDuringTranslate() {
        NoTranslatorFoundException exception = assertThrows(NoTranslatorFoundException.class, () -> Main.main(new String[] { "-translate", "-query", "SELECT 1", "-source", "invalid1", "-target", "invalid2" }));
    ...
    public void validationFailsForDumpWithArgumentWithoutValue() {
        MissingArgumentException exception = assertThrows(MissingArgumentException.class, () -> Main.isConfigurationInvalid(Main.parseArguments(new String[] { "-dump", "-database" })));
    ...
    public void validationFailsForTranslateWithArgumentWithoutValue() {
        MissingArgumentException exception = assertThrows(MissingArgumentException.class, () -> Main.isConfigurationInvalid(Main.parseArguments(new String[] { "-translate", "-query" })));
